...
export async function GET(req: NextRequest) {
  try {
    ...
    const permissions = {
      manage_roles: token
        ? EdgePermissionService.hasPermissionForToken(token, 'manage_roles')
        : false,
      manage_permissions: token
        ? EdgePermissionService.hasPermissionForToken(token, 'manage_permissions')
        : false,
      user_management: token
        ? EdgePermissionService.hasPermissionForToken(token, 'user_management')
        : false,
