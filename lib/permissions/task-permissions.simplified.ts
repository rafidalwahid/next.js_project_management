...
export async function checkTaskPermission(
  ...
  if (action === 'view') {
    // Anyone who is a team member or assignee can view
    hasPermission = isAdmin || isManager || isProjectCreator || isTeamMember || isAssignedToTask || isTaskAssignee;
  } else if (action === 'update') {
    // Admins, managers, project creators, and assignees can update
    hasPermission = isAdmin || isManager || isProjectCreator || isTeamMember || isAssignedToTask || isTaskAssignee;
  } else if (action === 'delete') {
    // Only admins, managers, and project creators can delete
