        ...
        <DialogContent className="w-[95vw] max-w-3xl max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 py-2 sm:py-4">
            <div className="grid gap-1 sm:gap-2">
              <Label htmlFor="title" className="text-sm sm:text-base">Project Title (minimum 3 characters)</Label>
              <Input
                id="title"
                name="title"
            ...
            <div className="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 md:grid-cols-3">
              <div className="grid gap-1 sm:gap-2">
                ...
                <Input
                  ...
                  value={projectData.startDate ? new Date(projectData.startDate.getTime() - (projectData.startDate.getTimezoneOffset() * 60000)).toISOString().split('T')[0] : ''}
                  onChange={(e) => {
              ...
              <div className="grid gap-1 sm:gap-2">
                ...
                <Input
                  ...
                  type="date"
                  className="w-full h-9 sm:h-10"
                  value={projectData.endDate ? new Date(projectData.endDate.getTime() - (projectData.endDate.getTimezoneOffset() * 60000)).toISOString().split('T')[0] : ''}
                  onChange={(e) => {
                    const date = e.target.value ? new Date(e.target.value) : null;
              ...
              <div className="grid gap-1 sm:gap-2">
                <Label htmlFor="estimatedTime" className="text-sm sm:text-base">Estimated Time (hours)</Label>
                <Input
                  id="estimatedTime"
                  name="estimatedTime"
                  type="number"
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-0">
                        ...
                        <div>
                          <div className="font-medium flex items-center text-sm sm:text-base">
                            {status.name}
                            {status.isDefault && (
                              <Badge className="ml-2 text-[10px] sm:text-xs" variant="outline">Default</Badge>
                            )}
                          </div>
                          {status.description && (
                            <div className="text-[10px] sm:text-xs text-muted-foreground">{status.description}</div>
                          )}
                        </div>
                      </div>
        ...
        <DialogContent className="w-[95vw] max-w-md p-4 sm:p-6">
          ...
          <div className="space-y-3 sm:space-y-4 py-2 sm:py-4">
            <div className="space-y-1 sm:space-y-2">
              <Label htmlFor="statusName" className="text-sm sm:text-base">Name</Label>
              <Input
                id="statusName"
            ...
            <div className="space-y-1 sm:space-y-2">
              <Label htmlFor="statusColor" className="text-sm sm:text-base">Color</Label>
            ...
            <div className="space-y-1 sm:space-y-2">
              <Label htmlFor="statusDescription" className="text-sm sm:text-base">Description (optional)</Label>
              <Textarea
            ...
            <div className="flex items-center space-x-2 mt-2">
              ...
              />
              <Label htmlFor="isDefault" className="text-xs sm:text-sm">Make this the default status for new tasks</Label>
            </div>
          </div>
