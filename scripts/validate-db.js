...
async function validatePrismaSchema() {
  ...
  try {
    ...
    if (!existsSync(migrationsDir)) {
      ...
      log('You may need to run "npx prisma migrate dev" to create migrations', colors.yellow);
    } else {
      // Count migrations
      const migrations = await fs.readdir(migrationsDir);
      log(`Found ${migrations.length} migration(s) in ${migrationsDir}`, colors.green);
    ...
    if (!existsSync(clientDir)) {
      ...
      log('You may need to run "npx prisma generate" to generate the client', colors.yellow);
      return false;
    }
